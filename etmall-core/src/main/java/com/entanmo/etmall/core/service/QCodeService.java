...
//    @Autowired
    ...
    public String createGrouponShareImage(String goodName, String goodPicUrl, EtmallGroupon groupon) {
        try {
            //创建该商品的二维码
            ...
            //存储分享图
            EtmallStorage storageInfo = storageService.store(inputStream2, imageData.length, "image/jpeg",
...
//            return "";
        ...
        try {
            ...
            ByteArrayInputStream inputStream2 = new ByteArrayInputStream(imageData);
            //存储分享图
            EtmallStorage EtmallStorage = storageService.store(inputStream2, imageData.length, "image/jpeg",
                    getKeyName(goodId));

            return EtmallStorage.getUrl();
        } catch (WxErrorException e) {
            logger.error(e.getMessage(), e);
        } catch (FileNotFoundException e) {
            logger.error(e.getMessage(), e);
