...
public class AdminAdminController {
    ...
    public Object list(String username,
        ...
        List<EtmallAdmin> adminList = adminService.querySelective(username, page, limit, sort, order);
    ...
    @PostMapping("/create")
    public Object create(@RequestBody EtmallAdmin admin) {
        ...
        List<EtmallAdmin> adminList = adminService.findAdmin(username);
        if (adminList.size() > 0) {
...
//        logHelper.logAuthSucceed("添加管理员", username);
    ...
    public Object read(@NotNull Integer id) {
        EtmallAdmin admin = adminService.findById(id);
        return ResponseUtil.ok(admin);
    }
    ...
    @PostMapping("/update")
    public Object update(@RequestBody EtmallAdmin admin) {
        Object error = validate(admin);
        if (error != null) {
            return error;
        }
...
//        logHelper.logAuthSucceed("编辑管理员", admin.getUsername());
    ...
    @PostMapping("/delete")
    public Object delete(@RequestBody EtmallAdmin admin) {
        Integer anotherAdminId = admin.getId();
        if (anotherAdminId == null) {
            return ResponseUtil.badArgument();
        }
        ...
        EtmallAdmin currentAdmin = (EtmallAdmin) currentUser.getPrincipal();
